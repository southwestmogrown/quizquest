// ---------------------------------------------------------------------------
// POST /api/complete — mark a reading lesson as complete, award XP, update streak.
//
// Accepts:  { courseSlug, lessonSlug }
// Returns:  { xpDelta, newTotalXp, newStreak, lessonCompleted } — HTTP 200
//           { error: string }                                    — HTTP 422 / 404
//
// Operates on the hard-coded demo user ("demo-user") for the MVP.
// See docs/mvp-functional-spec.md §4, §7.2.
// ---------------------------------------------------------------------------
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const { courseSlug, lessonSlug } = (body ?? {}) as Record<string, unknown>;
  ...
  try {
    ...
    if (lesson.type !== "reading") {
      return NextResponse.json(
        { error: `Lesson "${trimmedLessonSlug}" is not a reading lesson` },
        { status: 422 }
      );
    }
    ...
    console.error("Error loading course for completion:", err);
    return NextResponse.json(
  ...
  const { xpDelta, newTotalXp, newStreak, lessonCompleted } =
    await db.$transaction(async (tx) => {
      ...
      const delta = computeXpDelta(xpReward, prevBestXp);
      const newBestXp = Math.max(prevBestXp, xpReward);

      // A reading lesson is "newly completed" if it was not already completed.
  ...
  // ----- 4. Return result -------------------------------------------------
  return NextResponse.json({ xpDelta, newTotalXp, newStreak, lessonCompleted });
}
